h1. Persist User Guide

h2. Table of Contents
	"Introduction":#introduction
	"Quickstart":#quickstart
	"Creating Persist instances":#creating
	"Annotations":#annotations
	"Mapping POJOs to tables":#pojos
	"POJOs not mapped to tables (@NoTable)":#notable
	"java.util.Map's":#maps
...
bc. Persist persist = new Persist(connection);
...
@Map@'s can be used to hold data from queries. Persist will convert values returned from the query to Java
...
h2(#creating). Creating Persist instances
...
h2(#pojos). Mapping POJOs to tables
...
h2(#notable). POJOs not mapped to tables (@NoTable)
