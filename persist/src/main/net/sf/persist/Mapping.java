...
public final class Mapping {
	...
	public Mapping(final DatabaseMetaData metaData, final Class objectClass, final NameGuesser nameGuesser)
			throws SQLException {
		...
		while (resultSet.next()) {
			...
			columnTypes.put(columnName, resultSet.getInt(5));
		}
		...
		for (String fieldName : fields) {
			final String columnName = getColumnName(objectClass, nameGuesser, annotationsMap, 
					columnsList, tableName, fieldName);
	...
	public int[] getColumnTypes(final String[] columnNames) {
		final int[] types = new int[columnNames.length];
		for (int i = 0; i < columnNames.length; i++) {
			final Integer type = columnTypes.get(columnNames[i]);
			if (type == null) {
				throw new RuntimeSQLException("Unknown column [" + columnNames[i] + "]");
			}
			types[i] = type;
		}
		return types;
	...
	private static String getTableName(final DatabaseMetaData metaData, final String schema, 
			final Class objectClass, final NameGuesser nameGuesser) throws SQLException {
		...
		if (tableAnnotation != null && !tableAnnotation.name().equals("")) {
			...
			final Set<String> guessedNames = nameGuesser.guess(className);
	...
	/**
	 * Check if the given name corresponds to a table in the database and returns the corresponding name with the
	 * capitalization returned by the database metadata
	 */
	private static String checkTableName(final DatabaseMetaData metaData, final String schema, 
	...
	private static String getColumnName(final Class objectClass, final NameGuesser nameGuesser,
		...
		if (annotation != null && !annotation.name().equals("")) {
			...
			final Set<String> guessedNames = nameGuesser.guess(fieldName);
			...
			if (columnName == null) {
				throw new RuntimeSQLException("Field [" + fieldName + "] from class [" + objectClass.getName()
						...
						+ "with the database, please annotate it with @NoPersist");
			}
		}
	...
	private static Map[] getFieldsMaps(final Class objectClass) {
		...
		for (String suffix : allMethods.keySet()) {
			...
			if (getterSetter[0] != null && getterSetter[1] != null) {
				...
				// if NoPersist is specified, don't use the field
				final net.sf.persist.annotations.NoPersist noPersistGetter = getterSetter[0]
						.getAnnotation(net.sf.persist.annotations.NoPersist.class);

				final net.sf.persist.annotations.NoPersist noPersistSetter = getterSetter[1]
						.getAnnotation(net.sf.persist.annotations.NoPersist.class);

				// check conflicting NoPersist and Column annotations
				if (noPersistGetter != null || noPersistSetter != null) {
					if (getterAnnotation != null || setterAnnotation != null) {
						throw new RuntimeSQLException("Field [" + fieldName + "] from class [" + objectClass.getName()
								+ "] has conflicting NoPersist and Column annotations");
					}
					continue;
				}
	...
	/**
	 * Returns the first entry from the provided collection that matches the provided string 
