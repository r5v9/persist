 ...
 * <p>
 * It is used when a class specifies a {@link net.sf.persist.annotations.NoTable NoTable} annotation, which means
 * the class is not mapped to a table in the database, and will be only used to store data from queries.
 */
public class NoTableMapping extends Mapping {
	...
	public NoTableMapping(Class objectClass, NameGuesser nameGuesser) {
		...
		// create columns map by iterating through all fields in the object class
		// if a field has a @Column annotation, use it, otherwise add all guessed names for the field in the map
		...
		for (String fieldName : gettersMap.keySet()) {
			...
			if (annotation!=null) {
				if (annotation.autoGenerated()==true) {
					throw new RuntimeSQLException("@Column(autoGenerated=true) is set for field [" + fieldName + "] of class [" 
	...
	/**
	 * Returns the field name associated with a given column.
	 * If a mapping can't be found, will throw a RuntimeSQLException.
	 */
	public String getFieldNameForColumn(String columnName) {
		...
		if (fieldName==null) {
			throw new RuntimeSQLException("Could map field for column [" + columnName + "] on class [" 
					+ objectClass.getCanonicalName() + "]. Please specify an explict @Column annotation for that column.");
		}
		return fieldName;
	}

	/**
	 * Returns the setter method associated with a given column.
	 * If a mapping can't be found, will throw a RuntimeSQLException.
	 * 
	 * @see Mapping
	 */
	...
	/**
	 * Returns the getter method associated with a given column.
	 * If a mapping can't be found, will throw a RuntimeSQLException.
	 * 
	 * @see Mapping
	 */
	...
	/**
	 * Checks if a given column name conflicts with an existing name in the columns map.
	 */
	private void checkNameConflicts(String fieldName, String column) {
		...
		if (existingFieldName!=null) {
			throw new RuntimeSQLException("Fields [" + fieldName + "] and [" + existingFieldName 
					...
					+ "] either from guessed names or anotations. Please specify @Column mappings for at least one of those fields");
		}
	}

	/**
	 * Checks if {@link net.sf.persist.annotations.NoTable NoTable} is present and if a conflicting
	 * {@link net.sf.persist.annotations.Table Table} is not present.
	 */
